...
/**
 * Resolve the actual model name for API calls from the request's env.
 * When the user configures a model mapping in their provider config (e.g. sonnet -> "MiniMax-M2.5"),
 * it is delivered via the request env (ANTHROPIC_DEFAULT_*_MODEL) from the extension-host settings API.
 * This function checks those env values and returns the mapped model name if configured.
 *
 ...
 * The 1M context window is selected by the SDK based on whether the
 ...
 * env value cannot force the 1M context window back on.
 *
 * @param {string} modelId - Internal model ID from frontend (e.g. 'claude-sonnet-4-6' or 'claude-sonnet-4-6[1m]')
 * @param {object} userEnv - The env object from the request (settings.env)
